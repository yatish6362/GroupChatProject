      ...
      <section className='flex h-screen w-full flex-col md:flex-row'>
        <div className='left flex h-full w-full flex-col bg-slate-300 relative md:w-90'>
          <header className='flex items-center justify-between w-full bg-white px-4 py-3 shadow-sm'>
            ...
            <button className='rounded-full p-2 transition hover:bg-slate-100' onClick={() => setIsSidePanelOpen(!isSidePanelOpen)}>
              <i className='ri-group-fill text-lg'></i>
            </button>
          </header>

          <div className='conversation-area flex grow flex-col'>
            <div className='message-box grow overflow-y-auto p-2'>
            ...
            <div className='inputField flex w-full border-t border-slate-200 bg-white'>
              <input type='text' placeholder='Enter message' className='grow border-none bg-white px-4 py-3 outline-none' />
              <button className='bg-slate-600 px-4 text-white transition hover:bg-slate-700'>
                <i className='ri-send-plane-fill'></i>
              </button>
            </div>

            <div className={`absolute top-0 left-0 flex h-full w-full flex-col gap-2 bg-slate-100 shadow-xl transition-all duration-300 ${isSidePanelOpen ? 'translate-x-0' : '-translate-x-full'}`}>
              <header className='flex w-full items-center justify-between bg-white px-4 py-2'>
                <h1 className='font-semibold text-lg'>Collaborators</h1>
                <button className='rounded-full p-2 transition hover:bg-slate-100' onClick={() => setIsSidePanelOpen(false)}>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-3 py-4'>
          <div className='w-full max-w-lg rounded-2xl bg-white p-4 shadow-xl sm:p-6'>
            <div className='flex items-center justify-between'>
              <div>
                <h3 className='text-lg font-semibold text-slate-800'>Add Collaborators</h3>
                <p className='text-sm text-slate-500'>Choose users and save their IDs.</p>
              </div>
              <button className='rounded-full p-2 text-slate-500 transition hover:bg-slate-100' onClick={() => setIsModalOpen(false)}>
              ...
              {collaborators?.map((user) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`h-5 w-5 rounded-full border-2 ${isSelected ? 'border-sky-500 bg-sky-500' : 'border-slate-300'}`}>
                      {isSelected && <i className='ri-check-line flex items-center justify-center text-xs text-white'></i>}
                    </div>
                  </button>
                )
              })}
